...
// Soft-delete only — a deactivated department disappears from selection dropdowns but stays
// resolvable so anyone already assigned to it keeps a real label/color instead of a blank badge.
const deactivateDepartment = asyncHandler(async (req: Request, res: Response) => {
  ...
  invalidateOrgDepartmentCache(orgId);
  res.json(new ApiResponse(200, department, 'Department deactivated'));
});
