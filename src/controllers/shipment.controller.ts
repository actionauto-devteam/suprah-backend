...
const createShipment = asyncHandler(async (req: Request, res: Response) => {
  ...
  const quote = await Quote.findOne({
    ...
    "vehicleId",
    "year make modelName vin stockNumber image location",
  );
  ...
  const populatedShipment = await Shipment.findOne({
    _id: shipment._id,
    organizationId: orgId,
  })
    .populate({
  ...
  if (autoDeleteQuote) {
    ...
    await Quote.findOneAndUpdate(
      { _id: quoteId, organizationId: orgId },
      { status: "booked" },
    );
  }
  ...
  if (_io)
    _io.to(`org:${orgId}`).emit("shipment:change", { action: "created" });
  ...
  logger.info(
    { shipmentId: shipment._id, trackingNumber },
    "Converted quote to shipment",
  );
});
...
const getShipments = asyncHandler(async (req: Request, res: Response) => {
  ...
  if (isCacheable) {
    ...
    if (cached) {
      return res.json(
        new ApiResponse(200, cached, "Shipments fetched successfully"),
      );
    }
  }
  ...
  const attachOrg = async (list: any[]) => {
    const uniqueOrgIds = [
      ...new Set(
        list.map((s: any) => s.organizationId?.toString()).filter(Boolean),
      ),
    ];
    const validOrgIds = uniqueOrgIds.filter((id) =>
      /^[0-9a-fA-F]{24}$/.test(String(id)),
    );
    const orgs = await Organization.find({ _id: { $in: validOrgIds } }).select(
      "name logoUrl",
    );
    ...
    return list.map((s: any) => ({
      ...(s.toJSON ? s.toJSON() : s),
      organization: orgMap.get(s.organizationId?.toString()) || {
        name: "Unknown Org",
      },
    }));
  };
  ...
  const total = search
    ...
    : await Shipment.countDocuments(filter);

  // Attach organization name to each shipment
  const uniqueOrgIds = [
    ...new Set(
      filteredShipments
        .map((s) => s.organizationId?.toString())
        .filter(Boolean),
    ),
  ];
  const validOrgIds = uniqueOrgIds.filter((id) =>
  ...
  const shipmentsWithOrg = await Promise.all(
    filteredShipments.map(async (s) => {
      const shipmentObj = s.toJSON ? s.toJSON() : s;

      // Sign proof of delivery URL if exists
      if (
        shipmentObj.proofOfDelivery?.imageUrl &&
        !shipmentObj.proofOfDelivery.imageUrl.startsWith("http")
      ) {
  ...
  res.json(
    new ApiResponse(200, responseData, "Shipments fetched successfully"),
  );
});
...
const updateShipment = asyncHandler(async (req: Request, res: Response) => {
  ...
  if (requestedPickupDate !== undefined) {
    updateData.requestedPickupDate = requestedPickupDate
      ? new Date(requestedPickupDate)
      : null;
  }
  if (scheduledPickup !== undefined) {
    updateData.scheduledPickup = scheduledPickup
      ? new Date(scheduledPickup)
      : null;
  }
  ...
  if (scheduledDelivery !== undefined) {
    updateData.scheduledDelivery = scheduledDelivery
  ...
  if (trailerTypeRequired !== undefined)
    updateData.trailerTypeRequired = trailerTypeRequired;
  if (vehicleCount !== undefined) updateData.vehicleCount = vehicleCount;
  if (isPostedToBoard !== undefined)
    updateData.isPostedToBoard = Boolean(isPostedToBoard);
  if (preDispatchNotes !== undefined)
    updateData.preDispatchNotes = preDispatchNotes;
  ...
  if (carrierPayAmount !== undefined)
    updateData.carrierPayAmount = carrierPayAmount;
  if (copCodAmount !== undefined) updateData.copCodAmount = copCodAmount;
  if (specialInstructions !== undefined)
    updateData.specialInstructions = specialInstructions;
  if (loadSpecificTerms !== undefined)
    updateData.loadSpecificTerms = loadSpecificTerms;
  if (desiredDeliveryDate !== undefined)
    updateData.desiredDeliveryDate = desiredDeliveryDate
      ? new Date(desiredDeliveryDate)
      : null;
  ...
  if (destinationContact !== undefined)
    updateData.destinationContact = destinationContact;
  ...
  const _io = getSocketIO();
  if (_io)
    _io.to(`org:${orgId}`).emit("shipment:change", { action: "updated" });
...
const deleteShipment = asyncHandler(async (req: Request, res: Response) => {
  ...
  if (existingQuote) {
    await Quote.findOneAndUpdate(
      { _id: shipment.quoteId, organizationId: orgId },
      { status: "accepted" },
    );
  ...
  logger.warn(
    { shipmentId: req.params.id, trackingNumber },
    "Shipment deleted",
  );
});
 ...
 */
const submitProofOfDelivery = asyncHandler(
  async (req: Request, res: Response) => {
    const userId = getUserId(req);
    ...
    if (
      ...
      throw new ApiError(
        403,
        "Only the assigned driver can submit proof of delivery",
      );
    }
    ...
    logger.info(
      { shipmentId: shipment._id, userId },
...
const getShipmentStats = asyncHandler(async (req: Request, res: Response) => {
  ...
  res.json(
    new ApiResponse(200, formattedStats, "Statistics fetched successfully"),
  );
});
