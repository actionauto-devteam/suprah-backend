 ...
 */
const getDeliverableShipments = asyncHandler(
  async (req: Request, res: Response) => {
    ...
    res.json(
      new ApiResponse(
        200,
        result,
        "Deliverable shipments fetched successfully",
      ),
    );
  },
);
 ...
 * Returns unconfirmed proof submissions for the organization.
 * Staff users can see all pending proofs; drivers are restricted to proofs submitted to them.
 */
const getPendingProofs = asyncHandler(async (req: Request, res: Response) => {
    ...
    if (
      ...
      try {
        ...
        if (signed)
          return {
            ...item,
            proofOfDelivery: { ...item.proofOfDelivery, imageUrl: signed },
          };
      } catch {
        /* non-fatal */
      }
  ...
  const combined = await Promise.all(
    [...shipments, ...normalizedLoads]
      .sort(
        (a: any, b: any) =>
          new Date(b.proofOfDelivery?.submittedAt).getTime() -
          new Date(a.proofOfDelivery?.submittedAt).getTime(),
      )
      .map(signUrl),
  );
...
const createPayout = asyncHandler(async (req: Request, res: Response) => {
  ...
  const payout = await DriverPayout.create({
    ...
    description:
      description ||
      `Driver payout for shipment ${shipment.trackingNumber || shipmentId}`,
    status: "processing",
    notes,
    createdBy: userId,
  });
  ...
  try {
    ...
    logger.error(
      { err: stripeError, payoutId: payout._id, driverId },
...
const getPayouts = asyncHandler(async (req: Request, res: Response) => {
  ...
  res.json(
    new ApiResponse(200, payouts, "Driver payouts fetched successfully"),
  );
});
 ...
 */
const initiateDriverOnboarding = asyncHandler(
  async (req: Request, res: Response) => {
    const userId = getUserId(req);
    ...
    if (user.role !== "driver")
      throw new ApiError(403, "Only drivers can connect a Stripe account");
    ...
    if (!accountId) {
      ...
      try {
        ...
        throw new ApiError(
          500,
          stripeErr?.message || "Failed to create Stripe account",
        );
      }
    ...
    const rawOrigin =
      config.frontendUrl || config.corsOrigin || "http://localhost:3000";
 ...
 */
const getDriverConnectStatus = asyncHandler(
  async (req: Request, res: Response) => {
    const userId = getUserId(req);
    const user = await User.findById(userId);
    ...
    if (!user.stripeConnectAccountId) {
      return res.json(
        new ApiResponse(
          200,
          { connected: false },
          "Driver has not connected a Stripe account",
        ),
      );
    }
    ...
    try {
      ...
      if (isNoSuchAccount) {
        ...
        return res.json(
          new ApiResponse(
            200,
            { connected: false },
            "Stripe account not found, please reconnect",
          ),
        );
      }
      ...
      console.error(
        "[driverPayout] Error retrieving Stripe account:",
        err?.message,
      );
      res.json(
        new ApiResponse(
