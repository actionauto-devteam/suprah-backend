 ...
 * Body: { count?, multiUse? }
 * Always creates customer-type invites — drivers are a shared platform-wide
 * pool now, so driver invite links are generated by super_admin only, via
...
export const generateInviteLinks = asyncHandler(async (req: Request, res: Response) => {
  ...
  const accountType: InviteAccountType = 'customer';
 ...
 * Body: { emails: string[] }
 * Always creates customer accounts — see generateInviteLinks above for why
 * driver accounts no longer go through this org-scoped path.
 */
...
export const validateInviteLink = asyncHandler(async (req: Request, res: Response) => {
  ...
  // Driver invites are platform-wide, generated by super_admin — no org to
  // look up or brand the invite with.
...
export const registerViaInvite = asyncHandler(async (req: Request, res: Response) => {
  ...
  if (accountType === 'driver') {
    // Drivers are a shared platform-wide pool — no org to assign, and the
    // account stays pending until the SUPRAH.AI super_admin approves it,
    // exactly like the normal /sign-up/driver flow.
    ...
    await authService.ensureDriverRequest(user as any);

    try {
      await emailService.sendEmail({
        to: normalizedEmail,
        subject: 'Driver application submitted',
        text: `Hello ${name.trim()},\n\nYour driver application has been submitted for review by SUPRAH.AI. We'll email you once it's approved.\n\nEmail: ${normalizedEmail}`,
        html: `
          <div style="font-family:sans-serif;max-width:480px;margin:0 auto">
            <h2 style="color:#111">Driver application submitted</h2>
            <p>Hello <strong>${name.trim()}</strong>,</p>
            <p>Your driver application has been submitted for review by SUPRAH.AI. We'll email you once it's approved.</p>
          </div>
        `,
      });
    } catch {
    ...
    res.status(201).json(new ApiResponse(201, { email: normalizedEmail, accountType, pending: true }, 'Driver application submitted'));
    return;
  }
...
export const getCustomers = asyncHandler(async (req: Request, res: Response) => {
  ...
  const filter: any = {
    role: roleFilter,
    organizationId: actor.organizationId,
    // Drivers are a shared platform-wide pool now (no organizationId, and
    // approved only by the SUPRAH.AI super_admin under /admin/drivers), so
    // this org-scoped list will no longer surface new drivers. Kept as-is
    // for any legacy org-tied driver rows and for the customer role, whose
    // isApproved always defaults true and isn't gated.
    isApproved: true,
  };
