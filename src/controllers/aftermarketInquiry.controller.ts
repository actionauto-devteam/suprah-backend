...
export const submitInquiry = asyncHandler(async (req: Request, res: Response) => {
  ...
  res.status(201).json(
    new ApiResponse(201, { inquiry, conversationId: conversation._id }, 'Inquiry submitted successfully')
  );
});
...
export const adminListInquiries = asyncHandler(async (req: Request, res: Response) => {
  ...
  const [inquiries, total] = await Promise.all([
    AftermarketInquiry.find(filter).sort({ createdAt: -1 }).skip(skip).limit(limitNum).lean(),
  ...
  res.json(
    new ApiResponse(
      200,
      { inquiries, pagination: { page: pageNum, limit: limitNum, total, totalPages: Math.ceil(total / limitNum) } },
      'Inquiries fetched'
    )
  );
});
