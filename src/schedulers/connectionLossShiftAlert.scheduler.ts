...
export async function runConnectionLossShiftAlertCheck(): Promise<{ notified: number; checked: number }> {
  ...
  for (const loc of candidates) {
    ...
    const userDoc = (
      loc.userModel === 'User'
        ? await User.findById(loc.userId).select('locationConsent fullName organizationId department locationRequiredOverride').lean()
        : await CrmUser.findById(loc.userId).select('locationConsent fullName organizationId department locationRequiredOverride').lean()
    ) as any;
    if (!userDoc) continue;

    // Per-department kill switch for the whole feature (Settings → Departments
    // → "Require Location for TimeProof"), or a per-user override on top of it.
    if (!(await isLocationRequiredForUser((loc.organizationId as any)?.toString(), userDoc.department, userDoc.locationRequiredOverride))) continue;
