...
const upload = multer({
  ...
  limits: { fileSize: SUPRA_SPACE_MAX_VIDEO_FILE_SIZE_BYTES, files: SUPRA_SPACE_MAX_UPLOAD_FILES },
});
...
const uploadFiles: RequestHandler = (req, res, next) => {
  upload.array('files', SUPRA_SPACE_MAX_UPLOAD_FILES)(req, res, (err: any) => {
    ...
    if (err instanceof multer.MulterError) {
      if (err.code === 'LIMIT_FILE_SIZE') return next(new ApiError(400, 'Each attachment must be 40 MB or smaller.'));
      if (err.code === 'LIMIT_FILE_COUNT') return next(new ApiError(400, `You can attach up to ${SUPRA_SPACE_MAX_UPLOAD_FILES} files.`));
...
// Single-image upload for channel avatars (field 'avatar')
const uploadAvatarImage: RequestHandler = (req, res, next) => {
  multer({ storage: multer.memoryStorage(), limits: { fileSize: 8 * 1024 * 1024, files: 1 } })
...
// File / voice uploads
const uploadMiddlewareChain = [uploadLimiter, uploadFiles, validateUploadedFiles, supraSpaceController.uploadAttachment] as const;
router.post('/conversations/:id/upload', ...uploadMiddlewareChain);
router.post('/:id/upload', ...uploadMiddlewareChain); // legacy path
