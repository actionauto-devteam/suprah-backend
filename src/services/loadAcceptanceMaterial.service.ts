  ...
  export function getLoadAcceptanceMaterialSnapshot(load: any) {
    ...
    const pricingForAcceptance =
      source.postType === "assign-carrier" && source.pricing
        ? {
            // For direct assignments, compensation is the manually-entered total.
            // Internal estimates and legacy load-board fields must not create
            // false compensation amendments.
  ...
  function pricingSummary(value: any, postType?: any) {
    if (!value) return "Not set";

    // Direct assignments intentionally have one authoritative compensation
    // amount. Historical $/mi or COP/COD values may still exist on older rows,
    // but they are not part of the direct-assignment pricing presentation.
    if (postType === "assign-carrier") {
      return `Total driver pay: ${money(value.carrierPayAmount)}`;
    }
  ...
  export function buildLoadMaterialChanges(beforeLoad: any, afterLoad: any) {
    ...
    return fields.flatMap((field) => {
      ...
      return [{
        ...
        before:
          field === "pricing"
            ? pricingSummary(before[field], before.postType)
            : summarize(field, before[field]),
        after:
          field === "pricing"
            ? pricingSummary(after[field], after.postType)
            : summarize(field, after[field]),
      }];
    });
  }