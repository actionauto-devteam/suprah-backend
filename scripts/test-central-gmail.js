...
async function testCentralGmail() {
    console.log('\n--- 🧪 Central Gmail Diagnostic Tool (JS Version) ---');
    ...
    try {
        ...
        // credentials contains { access_token, refresh_token, expiry_date, token_type, id_token }
        const res = await oauth2Client.refreshAccessToken();
        const credentials = res.credentials;

        console.log('✅ Token refresh successful!');
        console.log('- New Access Token: ' + (credentials.access_token ? credentials.access_token.substring(0, 10) + '...' : 'N/A'));
        console.log('- Expiry: ' + (credentials.expiry_date ? new Date(credentials.expiry_date).toLocaleString() : 'N/A'));
        ...
        const msgRes = await gmail.users.messages.list({ userId: 'me', maxResults: 5 });
        ...
        if (error.message.includes('invalid_grant')) {
            ...
            console.log('2. Generate a FRESH refresh token.');
